...
class Job(DataClass):
    ...
    def __post_init__(self) -> None:
        ...
        if not self.passport:
            self.passport = Passport(data_type=self.project.data_type)
        if not self.description:
            self.description = f"Job to obtain {self.project.data_type.value} for the {self.project.category.display_name} category."  # type: ignore
