...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.14                                                                             #
# Filename   : /appstorestream/domain/base/state.py                                                #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
# Modified   : Monday July 29th 2024 02:11:20 pm                                                   #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
class StateManager(ABC):
    ...
    def add_response(self, response: AsyncResponse) -> None:
        ...
        self.responses.append(
            (current_time, response.request_count, response.total_errors)
        )
    ...
    def _clean_responses(self) -> None:
        ...
        while (
            self.responses
            and (datetime.now() - self.responses[0][0]).total_seconds()
            > self.window_size
        ):
...
class StateManagerShortCircuit404s(StateManager):
    ...
    def add_response(self, response: AsyncResponse) -> None:
        ...
        current_time = datetime.now()
        self.responses.append(
            (current_time, response.request_count, response.page_not_found_errors)
        )
        self._clean_responses()
...
class CircuitBreaker(Service):
    ...
    def __init__(
        ...
        self._short_circuit_errors_failure_rate_threshold = (
            short_circuit_errors_failure_rate_threshold
        )
        self._short_circuit_404s_window_size = short_circuit_404s_window_size
        self._short_circuit_404s_failure_rate_threshold = (
            short_circuit_404s_failure_rate_threshold
        )
        ...
        self._closed_state_manager = StateManagerClosed(
            window_size=self._closed_window_size,
            failure_rate_threshold=self._closed_failure_rate_threshold,
        )
        self._half_open_state_manager = StateManagerHalfOpen(
            window_size=self._half_open_window_size,
            failure_rate_threshold=self._half_open_failure_rate_threshold,
        )
        self._short_circuit_error_state_manager = StateManagerShortCircuitErrors(
            window_size=self._short_circuit_errors_window_size,
            failure_rate_threshold=self._short_circuit_errors_failure_rate_threshold,
        )
        self._short_circuit_404_state_manager = StateManagerShortCircuit404s(
            window_size=self._short_circuit_404s_window_size,
            failure_rate_threshold=self._short_circuit_404s_failure_rate_threshold,
        )
    ...
    def evaluate_response(self, response: AsyncResponse) -> None:
        ...
        if self._short_circuit_error_state_manager.failure_rate_exceeded(
            response=response
        ):
        ...
        if self._short_circuit_404_state_manager.failure_rate_exceeded(
            response=response
        ):
    ...
    def _burnin(self) -> bool:
        ...
        ).total_seconds() < self._closed_burnin_period
    ...
    def _half_open_circuit(self) -> None:
        ...
        self._logger.info(
            "Circuit breaker half-opened. Introducing delays between requests."
        )
    ...
    def _terminate_circuit(self) -> None:
        ...
        self._job.terminate()
        self._logger.info(
            "Circuit breaker terminated. No further requests will be made."
        )
    ...
    def _complete_circuit(self) -> None:
        ...
        self._job.complete()
        self._logger.info(
            "Circuit breaker marked job as complete. No further requests will be made."
        )
