...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.14                                                                             #
# Filename   : /tests/test_infra/test_database/test_dba.py                                         #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
SCRIPT_FILEPATH = "scripts/database/setup/database/create.sh"
DDL_DIRECTORY = "scripts/database/setup/tables"
DDL_FILEPATH = "scripts/database/setup/tables/02_config.sql"
TABLENAME = 'config'

@pytest.mark.dba
class TestDBA:  # pragma: no cover
    # ============================================================================================ #
    def test_drop_database_exists(self, caplog):
        start = datetime.now()
        logger.info(
    ...
    # ============================================================================================ #
    def test_create_database_exists(self, caplog):
        start = datetime.now()
        logger.info(
            f"\n\nStarted {self.__class__.__name__} {inspect.stack()[0][3]} at {start.strftime('%I:%M:%S %p')} on {start.strftime('%m/%d/%Y')}"
        )
